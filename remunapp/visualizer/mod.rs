...
fn font_setup(ctx: &egui::Context) {
    ...
    fonts.families.insert(
        egui::FontFamily::Name("Helvetica".into()),
...
impl Visualizer {
    ...
    pub fn update(&mut self, ctx: &egui::Context, state: &mut State) {
        ...
        egui::SidePanel::left("left_bar")
            .frame(frame)
            .show(ctx, |ui| {
                if let Some(metadata) = state.ines.metadata.as_ref()
                    && let Some(data_source) = metadata.data_source.as_ref()
                {
                ...
                if self.running {
                    ...
                    for _ in 0..instructions_to_run {
                        ...
                        if let Some(m) = state.ines.metadata.as_ref()
                            && m.breakpoints.contains(&state.pc)
                        {
                            self.running = false;
                            break;
                        }
...
fn scroll_area(ctx: &egui::Context, ui: &mut egui::Ui, scroll: &mut usize, cursor: &mut u64) {
    ...
    if new_line_number >= 0 {
        *scroll = new_line_number as usize
    }
}
